...
class GspeechClient(Node):
    def __init__(self):
        ...
        # ROS Text Publisher
        self._results_pub = self.create_publisher('/google_client/text', DialogflowResult,
        #                                     queue_size=10)
        # Context clues in yaml file
        yamlFileDir = str(pathlib.Path(__file__).parent.resolve()).removesuffix('/dialogflow_ros2') + '/config/context.yaml'
    ...
    def _listen_print_loop(self, responses):
        ...
        try:
            for response in responses:
                ...
                if result.is_final:
                    self.get_logger().debug("Google Speech result: {}".format(transcript))
            ...
            self.get_logger().warn("{} caught in Mic. Client".format(e))
            self.gspeech_client()
    ...
    def start_client(self):
        ...
        try:
            self.get_logger().info(("Google STT started")
            self.gspeech_client()
